...
public class MypageViewHandler {
    ...
    @StreamListener(KafkaProcessor.INPUT)
    public void whenSpecialOrdered_then_CREATE_2 (@Payload SpecialOrdered specialOrdered) {
        try {
            if (specialOrdered.isMe()) {
                // view 객체 생성
                Mypage mypage = new Mypage();
                // view 객체에 이벤트의 Value 를 set 함
                mypage.setOrderId(specialOrdered.getId());
                mypage.setProductId(specialOrdered.getProductId());
                mypage.setQty(specialOrdered.getQty());
                mypage.setStatus(specialOrdered.getStatus());
                // view 레파지 토리에 save
                mypageRepository.save(mypage);
            }
        }catch (Exception e){
